...
public class HeaderPositionCalculator {
     ...
     */
    protected Rect mTempRect1 = new Rect();
    protected Rect mTempRect2 = new Rect();
    ...
    public boolean hasNewHeader(int position, boolean isReverseLayout) {
        if (indexOutOfBounds(position)) return false;
        ...
        if (!indexOutOfBounds(nextItemPosition)) nextItemHeaderId = mAdapter.getHeaderId(nextItemPosition);
    ...
    protected void initDefaultHeaderOffset(Rect headerMargins, RecyclerView recyclerView, View header, View firstView, int orientation) {
    ...
    protected boolean isStickyHeaderBeingPushedOffscreen(RecyclerView recyclerView, View stickyHeader) {
        ...
        if (firstViewUnderHeaderPosition == RecyclerView.NO_POSITION) return false;
        ...
        if (firstViewUnderHeaderPosition > 0 && hasNewHeader(firstViewUnderHeaderPosition, isReverseLayout)) {
            ...
            if (mOrientationProvider.getOrientation(recyclerView) == LinearLayoutManager.VERTICAL) {
                ...
                if (topOfNextHeader < bottomOfThisHeader) return true;
            } else {
                ...
                if (leftOfNextHeader < rightOfThisHeader) return true;
            }
        }
    ...
    protected void translateHeaderWithNextHeader(RecyclerView recyclerView, int orientation, Rect translation,
        ...
        if (orientation == LinearLayoutManager.VERTICAL) {
            ...
            if (shiftFromNextHeader < topOfStickyHeader) translation.top += shiftFromNextHeader;
        } else {
            ...
            if (shiftFromNextHeader < leftOfStickyHeader) translation.left += shiftFromNextHeader;
        }
    }
     ...
     */
    protected View getFirstViewUnobscuredByHeader(RecyclerView parent, View firstHeader) {
        ...
        for (int i = from; i >= 0 && i <= parent.getChildCount() - 1; i += step) {
            ...
            if (!itemIsObscuredByHeader(parent, child, firstHeader, mOrientationProvider.getOrientation(parent))) return child;
        }
        return null;
    }
    ...
    protected boolean itemIsObscuredByHeader(RecyclerView parent, View item, View header, int orientation) {
        ...
        if (orientation == LinearLayoutManager.VERTICAL) {
            ...
            if (itemTop >= headerBottom) return false;
        } else {
            ...
            if (itemLeft >= headerRight) return false;
        }

        return true;
    }

    protected int getListTop(RecyclerView view) {
    ...
    protected int getListLeft(RecyclerView view) {
        if (view.getLayoutManager().getClipToPadding()) {
